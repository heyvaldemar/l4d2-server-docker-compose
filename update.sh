...
COMPOSE_FILE="l4d2-server-docker-compose.yml"
PROJECT="${COMPOSE_PROJECT_NAME:-l4d2}"
DRY_RUN=false
ALLOW_MAJOR=false
for arg in "$@"; do
  case "$arg" in
...
if [ "$new_major" != "$cur_major" ] && [ "$ALLOW_MAJOR" != "true" ]; then
  ...
  echo "  https://github.com/heyvaldemar/l4d2-server-docker-compose/releases/tag/$latest" >&2
  exit 3
fi
